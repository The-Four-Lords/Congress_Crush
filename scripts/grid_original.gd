...
func reset_pieces_in_combo():
	pieces_in_combo = 0

func sound_acording_pieces_in_combo():
	print("Combo de "+String(pieces_in_combo)+", numero de combos "+String(combos_in_move)+", total de piezas destruidas "+String(pieces_destroyed)+" state["+String(state)+"]")
	pass
				...
				if i > 0 && i < width - 1:
					if not all_pieces[i-1][j]==null && not all_pieces[i+1][j]==null:
						if all_pieces[i-1][j].color == current_color && all_pieces[i+1][j].color == current_color:
							...
							pieces_in_combo = visibility_pieces_changed
							combos_in_move += 1	
				...
				if j > 0 && j < height - 1:
					if not all_pieces[i][j-1]==null && not all_pieces[i][j+1]==null:
						if all_pieces[i][j-1].color == current_color && all_pieces[i][j+1].color == current_color:
							...
							pieces_in_combo += visibility_pieces_changed
							combos_in_move += 1
	pieces_destroyed += pieces_in_combo	
	sound_acording_pieces_in_combo()
	reset_pieces_in_combo()
